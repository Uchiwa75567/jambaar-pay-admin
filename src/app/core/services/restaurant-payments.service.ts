...
import { normalizePhone } from '../utils/form-validation';
...
export class RestaurantPaymentsService {
  ...
  readonly qrCodeUrl = computed(() => buildQrCodeUrl(RESTAURANT_QR_PHONE_NUMBER));
...
function buildQrCodeUrl(value: string): string {
  const payload = encodeURIComponent(value.trim());
  return `https://api.qrserver.com/v1/create-qr-code/?size=${QR_CODE_SIZE}x${QR_CODE_SIZE}&margin=0&data=${payload}`;
}
